...
export default class BorrarCliente extends Component {
  constructor() {
    super();
    this.state = {
      clientes: [],
      clienteBorradoId: 0,
      proveedorModificadoId: 0
    }
  }
  componentDidMount = () => {
  ...
  borrarBoton = (id) => {
    ...
    axios.post(url + '/cliente/borrar?id=' + id)
      .then(response => {
            ...
            <tbody>
              {
                this.state.clientes.map(elemento => (
                  <tr key={elemento.id} value={elemento} id={elemento.id}>
                    ...
                    value={elemento.id} id={elemento.id}
                      name={elemento.id} onClick={() => this.borrarBoton(elemento.id)}>
                        Borrar
                      </ReactBoostrap.Button></td>
                    <td>
                      <Link className='btn btn-primary' to={'/ModificarCliente/' + elemento.id} >
                        Modificar
                      </Link>
                    </td>
                  </tr>
